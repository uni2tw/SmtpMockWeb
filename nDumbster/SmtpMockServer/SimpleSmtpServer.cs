...
namespace nDumbster.SmtpMockServer
{
	...
	public class SimpleSmtpServer : IDisposable
	{
		...
		/// </summary>
		private ArrayList receivedMail = new ArrayList();
		...
		virtual public SmtpMessage[] ReceivedEmail
		{
			get
			{
				lock(this)
				{
					return (SmtpMessage[])receivedMail.ToArray(typeof(SmtpMessage));
				}
			}
		}
		...
		private IList HandleSmtpTransaction(StreamWriter output, StreamReader input)
		{
			...
			while (smtpState != SmtpState.CONNECT)
			{
				...
				if (smtpState == SmtpState.QUIT)
				{
				    ...
				    {
                        MailMessageWrapper mmp = MailMessageWrapper.ConvertFrom(msg);
                        OnMessageReceived(mmp);
				    }
				    msg = new SmtpMessage();
				}
