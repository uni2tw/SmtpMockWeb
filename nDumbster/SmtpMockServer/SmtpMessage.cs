			...
			if (commandData != null)
			{
				if (SmtpState.DATA_HDR == response.NextState)
				{
					...
					if (headerNameEnd >= 0)
					{
					    ...
					    value_Renamed = Attachment.CreateAttachmentFromString("", value_Renamed).Name;
                        headers.Add(name, value_Renamed);
                        prevName = name;
                    }
				    if (headerNameEnd == -1 && prevName != null)
				    {
				        headers[prevName] += Attachment.CreateAttachmentFromString("", commandData).Name;
				        prevName = null;
				    }
				}
